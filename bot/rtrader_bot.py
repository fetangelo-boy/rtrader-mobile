...
async def cmd_start(message: Message):
    """Команда /start — приветствие или продление подписки."""
    # Проверяем deep link параметр (например /start renew)
    args = message.text.strip().split(maxsplit=1)
    deep_link_param = args[1] if len(args) > 1 else ""

    if deep_link_param == "renew":
        # Пользователь пришёл из кнопки "Управление подпиской" в приложении
        await message.answer(
            "🔄 Продление подписки RTrader\n\n"
            "Для продления подписки отправьте скриншот чека об оплате "
            "в этот чат.\n\n"
            "После проверки администратором ваша подписка будет продлена."
        )
    else:
        await message.answer(
