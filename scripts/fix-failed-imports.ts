...
async function fixImports() {
  ...
  console.log('[Fix] Starting fix for failed imports...\n');
  ...
  // Get system user ID
  const systemUsers = await db.select().from(authUsers).where(
    (col: any) => col.email === systemEmail
  );
  ...
  // Step 2: Re-import failed messages
  ...
  for (const msg of messagesData) {
    ...
    if (isNaN(userId)) {
      // This is a failed message, re-import with system user
      const chatId = parseInt(msg.chat_id);
      try {
        await db.insert(messages).values({
          chatId,
          userId: systemUserId as any,
          content: msg.content,
  ...
  // Step 3: Re-import failed participants
  ...
  for (const part of participantsData) {
    ...
    if (isNaN(userId)) {
      // This is a failed participant, re-import with system user
